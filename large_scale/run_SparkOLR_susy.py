...
MODEL_NAME = "SparkOLR_susy"
D = 18
K = 2
NUM_TRAIN_PARTITIONS = 225
NUM_TEST_PARTITIONS = 50
...
#     data = sc.textFile(join(DATA_DIR, DATA_FILE_TRAIN), NUM_TRAIN_PARTITIONS).mapPartitions(read_point_batch).map(lambda batch: scale_data(batch[0], batch[1]))
    ...
    pred_lab = data_test.map(lambda batch: (sigmoid(batch[0].dot(w.T)).argmax(axis=1), batch[1])).cache()
